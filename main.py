...
def agregarReceta():
    ...
    receta = Receta(id, datos['autor'], datos['titulo'], datos['resumen'], datos['ingredientes'], datos['procedimiento'], datos['tiempo'], datos['imagen'])
...
def agregarRecetas():
    ...
    for row in reader:
        receta = Receta(id, row[0], row[1], row[2], row[3], row[4], row[5], row[6])
        recetas.append(receta)
        id += 1
    return {"msg": 'Receta agregada'}
